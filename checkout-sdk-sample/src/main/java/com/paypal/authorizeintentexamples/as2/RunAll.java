...
public class RunAll {
	public static void main(String[] args) {
		try {
			...
			orderResponse = new SaveOrder().saveOrder(orderId, false);
			String oId = "";
			if (orderResponse.statusCode() == 201) {
				System.out.println("Saved Successfully\n");
				oId = orderResponse.result().purchaseUnits().get(0).id();
			}
			System.out.println("O-Id:" + oId);
			...
			HttpResponse<Capture> captureOrderResponse = new CaptureOrder().captureOrder(authId, false, false);
			if (orderResponse.statusCode() == 201) {
			...
			// Perform Second Authorization
			System.out.println("Authorizing Order...");
			orderResponse = new AuthorizeOrder().authorizeOrder(orderId, false);
			if (orderResponse.statusCode() == 201) {
				System.out.println("Authorized Successfully\n");
				List<Authorization> authorizations = orderResponse.result().purchaseUnits().get(0).payments()
						.authorizations();
				authId = authorizations.get(authorizations.size() - 1).id();
			}

			// Perform capture on second Authorization
			System.out.println("Capturing Order...");
			captureOrderResponse = new CaptureOrder().captureOrder(authId, false, false);
			if (orderResponse.statusCode() == 201) {
				System.out.println("Captured Successfully");
				System.out.println("Status Code: " + captureOrderResponse.statusCode());
				System.out.println("Status: " + captureOrderResponse.result().status());
			...
			// Perform capture on third Authorization
			System.out.println("Capturing Order...");
			captureOrderResponse = new CaptureOrder().captureOrder(authId, false, false);
			if (orderResponse.statusCode() == 201) {
				System.out.println("Captured Successfully");
			...
			System.out.println("Capturing Order...");
			captureOrderResponse = new CaptureOrder().captureOrder(authId, true, false);
			if (orderResponse.statusCode() == 201) {
				System.out.println("Captured Successfully");
				System.out.println("Status Code: " + captureOrderResponse.statusCode());
				System.out.println("Status: " + captureOrderResponse.result().status());
			...
			HttpResponse<Refund> refundHttpResponse = new RefundOrder().refundOrder(captureOrderResponse.result().id(),
					false);
